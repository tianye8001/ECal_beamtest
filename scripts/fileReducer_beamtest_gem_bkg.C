...
//	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem_Etotalcut_1GeV.root",the_filename), "RECREATE");
...
//	outTree->Branch("Cer_n",      &Cer_n,   "Cer_n/I"    ); //total hits on the virtual plane of GEM00 
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		/*	else if(flux_id->at(j)==13) hit_id=10; // 
			...
			if(flux_tid->at(j)==1 ){
				//cout<<"event="<<i<<"tid="<<flux_tid->at(j)<<"pid="<<flux_pid->at(j)<<endl;
				if(hit_id==6){
                                Is_prime=1;
					p_max=hit_pf;
			...
			//cout<<"event="<<i<<"j="<<j<<" GEM00_n="<<GEM00_n<<"GEM10_n="<<GEM10_n<<endl;
                ...
                // process gem
                //if(Is_prime==1){
		//	cout<<"event="<<i<<"prime_particle="<<Is_prime<<endl;
                //}
