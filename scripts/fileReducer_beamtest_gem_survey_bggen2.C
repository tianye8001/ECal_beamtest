...
int fileReducer_beamtest_gem_survey_bggen2(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
//	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem1_Etotalcut_40uA_Q210MeV.root",the_filename), "RECREATE");
	//TFile* outFile = new TFile(Form("%s_reduce_tree_rate_CC_eEcut.root",the_filename), "RECREATE");
	TFile* outFile = new TFile("/volatile/halla/solid/tianye/container/HallC_beamtest_bggen_LH2_pip_7degsurvey_Cfoil/reduce_tree_rate_Nphe_Certest.root", "RECREATE");
