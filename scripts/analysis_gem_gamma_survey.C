...
int analysis_gem_gamma_survey(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_SCAD_gammaE_SCAsignal.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if (hit_id==0) {//gem1
				...
				N_gem00 ++;
                              //  if(i==13580){
				//cout<<"event="<<i<<"index="<<j<<"hit_ID="<<hit_id<<" hit_gem00"<<"pid="<<flux_pid->at(j)<<"pf="<<Edep<<endl;
                                //}
		...
		if(Fillecal){
			...
			if(true){
				...
				for(int sensor_id=0;sensor_id<ch_hgc;sensor_id++){
					...
					//if(Eend_ec_ps_sum>0){
					if( Eend_ec_ps[2]>0){
		                        // cout<<"event="<<i<<"Edep="<<Eend_ec[2]<<"Eend_ec_ps="<<Eend_ec_ps[2]<<"rate="<<rate<<endl;
