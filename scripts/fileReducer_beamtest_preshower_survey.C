...
int fileReducer_beamtest_preshower_survey(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_LASPD_Etotalcut_positron.root",the_filename), "RECREATE");
...
//	outTree->Branch("Cer_n",      &Cer_n,   "Cer_n/I"    ); //total hits on the virtual plane of GEM00 
	...
	double Ec=0;
	double trigger_ec=0;
	int hit_id=-1,pid_max=0,mpid_max=0,tid_max=0,mtid_max=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		//	else if(flux_id->at(j)==30) hit_id=16; //
				...
				if(hit_id==8 /*&& flux_pid->at(j)==22 && flux_tid->at(j)==1 && flux_mtid->at(j)==0*/){
                                Is_prime=1;
		                N_preshower ++;
				if (E >= edep_6p1_max){
					edep_6p1_max=E;
					...
					pz_max=flux_pz->at(j);
                                        ...
                                        tid_max = flux_tid->at(j);
                                        mtid_max = flux_mtid->at(j);
                                 }
                                //cout<<"event="<<i<<"flux="<<j<<"pid="<<pid_max<<"mpid="<<mpid_max<<"mtid="<<mtid_max<<"tid="<<tid_max<<endl;
				} 
				if(hit_id==0 ){
					SC_A_p_max=hit_pf;
				}
			...
			//cout<<"event="<<i<<"j="<<j<<" GEM00_n="<<GEM00_n<<"GEM10_n="<<GEM10_n<<endl;
                ...
                // process gem
              if(Is_prime==1 &&  pid_max==-11 && mpid_max==22 && mtid_max==3 && tid_max==5){
