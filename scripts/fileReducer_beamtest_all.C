...
int fileReducer_beamtest_all(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_Etotalcut.root",the_filename), "RECREATE");
...
//	ofstream outfile1(Form("%s_output.csv",the_filename));
	...
	int ecN=0;
	float PreShP,PreShPx,PreShPy,PreShPz,PreShSum, PreSh_l, PreSh_r, PreSh_t,PreShtheta;
	//  float ShP,ShPx,ShPy,ShPz;
	float ShowerSum, Shower_l, Shower_r, Shower_t;
	float LASPD_Eendsum, SPD_Eendsum, SC0_Eendsum, SC1_Eendsum, SC2_Eendsum;
	float LASPD_Eend, SPD_Eend, SC0_Eend, SC1_Eend, SC2_Eend;
	float SC0_P;
	float SC1_P;
	float SC2_P;
	float SPD_P;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			//cout<<"event="<<i<<"j="<<j<<" GEM00_n="<<GEM00_n<<"GEM10_n="<<GEM10_n<<endl;
                ...
                // process gem
              //if(Is_prime==1 /*&& p_max>200*/){

		//cout<<"event="<<i<<"pid="<<pid_max<<"p_max="<<p_max<<endl;
                //}
