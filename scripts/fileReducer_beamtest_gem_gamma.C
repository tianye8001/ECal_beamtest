...
int fileReducer_beamtest_gem_gamma(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem1_Etotalcut_gamma.root",the_filename), "RECREATE");
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		//	else if(flux_id->at(j)==30) hit_id=16; //
			...
			//cout<<"event="<<i<<"j="<<j<<" GEM00_n="<<GEM00_n<<"GEM10_n="<<GEM10_n<<endl;
                ...
                // process gem
              if(Is_prime==1 &&  N_preshower<=1 && edep_6p1_max==400){
