...
int analysis_hadron(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_Nphe.root",the_filename);
	...
	TH2F *hhit_vR_vs_vz_cutr[n],*hhit_R_vs_z_cutr[n],*hhit_vR_vs_vz_cute[n],*hhit_R_vs_z_cute[n],*gen_theta_vs_p_cutr[n],*gen_theta_vs_p_cute[n],*vertexZ_vs_p_hist[n],*vertexZ_vs_p_cut_hist[n],*vertexZ_vs_p_cute_hist[n],*vertexZ_vs_p_cutr_hist[n]; 
	for(int i=0;i<n;i++){
		char hstname[100];
		...
		sprintf(hstname,"hit_p_vs_vz_%i",i);
		vertexZ_vs_p_hist[i]=new TH2F(hstname,detname[i].c_str(),1300,-100,1200,1100,0,11);        
		sprintf(hstname,"hit_p_vs_vz_cut_%i",i);
		vertexZ_vs_p_cut_hist[i]=new TH2F(hstname,detname[i].c_str(),1300,-100,1200,1100,0,11);        
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		if (filemode=="BeamOnTargetEM" || filemode=="BeamOnTarget") {
			rate=60e-6/1.6e-19/event_actual/loop_time*add_norm;  //1ua		  
		...
		//	hhit_Ec[hit_id]->Fill(Ec,rate);
			...
			/*	hhit_vR_vs_vz_cut[hit_id]->Fill(vz_max,vr_max,rate);
				...
				if(pid_gen!=111){ 
					...
					if (flux_tid->at(j) ==1 ){
						...
						vertexZ_vs_p_cut_hist[hit_id]->Fill(hit_vz,hit_p,rate);
			...
			if(BOT==1){
				...
				vertexZ_vs_p_cut_hist[id_max]->Fill(vz_max,p_max,rate);
                        }
		//fill every events
