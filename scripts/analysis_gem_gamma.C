...
int analysis_gem_gamma(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_gem_gammaE_pzcut_debug.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if (hit_id==4) {//gem1
				...
				N_gem00 ++;
				if(flux_pid->at(j)==22 && flux_pz->at(j)>0 /*&& abs(hit_vz)<5*/){  
