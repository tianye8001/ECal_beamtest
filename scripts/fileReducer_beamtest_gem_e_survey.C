...
int fileReducer_beamtest_gem_e_survey(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem1_Etotalcut_e.root",the_filename), "RECREATE");
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		//	else if(flux_id->at(j)==30) hit_id=16; //
			...
			//if(flux_tid->at(j)==1 ){
				...
				if(hit_id==4 && flux_mtid->at(j)==1 && flux_mpid->at(j)==22 && flux_tid->at(j)<=3){
				//if(hit_id==3 && abs(flux_avg_lx->at(j)/1e1)<2.5 && abs(flux_avg_ly->at(j)/1e1)<5){
					...
					pz_max_e=flux_pz->at(j);
                                ...
                                //cout<<"event="<<i<<"htid="<<hit_id<<"pid="<<flux_pid->at(j)<<"p_max="<<p_max<<"pf_hit="<<hit_pf<<"edep_6p1_max="<<edep_6p1_max<<endl;
                                //cout<<"event="<<i<<"htid="<<hit_id<<"pid="<<flux_pid->at(j)<<"p_max="<<p_max<<"pf_hit="<<hit_pf<<"edep_6p1_max="<<edep_6p1_max<<"N_preshower="<<N_preshower<<endl;
				} 
				if(hit_id==0 ){
					SC_A_p_max=hit_pf;
				}
			...
			//cout<<"event="<<i<<"j="<<j<<" GEM00_n="<<GEM00_n<<"GEM10_n="<<GEM10_n<<endl;
             ...
             if(Is_prime==1 && p_max>0 && p_max_e>0/*&& edep_6p1_max<0.06&& p_max>200*/){
                //cout<<"event="<<i<<"preshower_N="<<N_preshower<<"E_max="<<edep_6p1_max<<endl;
