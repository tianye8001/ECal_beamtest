...
int analysis_gem_survey_theta_plot(string inputfile_name,string runmode="trigger",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_preshower_theta_rate_SCB_LASPD.root",the_filename);
	...
	TH2F *hgen_ThetaP=new TH2F("gen_ThetaP","generated events;vertex #theta (deg);vertex P (GeV)",180,0,90,110,0,11);  
	TH2F *hgen_ThetaP_cut=new TH2F("gen_ThetaP_cut","generated events;vertex #theta (deg);vertex P (GeV)",180,0,90,110,0,11);  
	TH2F *hgen_ThetaEdep_cut=new TH2F("gen_ThetaEdep_cut","generated events;vertex #theta (deg);Shower Edep (MeV)",180,0,90,250,0,1000);  
	...
	TH2F *vertexZ_vs_theta_hist=new TH2F("vertexmZ_vs_theta_hist","vertex;mz (cm);hit #theta(deg)",6480,-40,3200,360,0,180);	
	TH2F *vertexZ_vs_theta_gem2_hist=new TH2F("vertexmZ_vs_theta_gem2_hist","vertex;mz (cm);hit #theta (deg)",6480,-40,3200,360,0,180);	
	...
	TH2F *hitZ_vs_hitR_hist=new TH2F("hitZ_vs_hitR_hist","gem1;z (cm);R (cm)",120,200,260,80,720,760);	
        //TH2F *gen_theta_p_hist =new TH2F("gen_theta_p_hist","generated;#theta;p (MeV)",180,0,90,1000,0,5000);
        //TH2F *gen_theta_p_cut_hist= new TH2F("gen_theta_cut_p_hist","generated;#theta;p (MeV)",180,0,90,1000,0,5000);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		pid_gen=0;
		theta_gen=0,phi_gen=0,p_gen=0,px_gen=0,py_gen=0,pz_gen=0,vx_gen=0,vy_gen=0,vz_gen=0;      
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if (hit_id==8 /*&& flux_tid->at(j)==1*/) {//gem1
			...
			if (hit_id==7 /*&& flux_tid->at(j)==1*/) {//EC front 
				if(flux_pz->at(j)>0){
					if (hit_p >= edep_6p1_max){
...
//					cout<<"event="<<i<<"index="<<j<<"Edep="<<trackE_flux_gem2<<endl;
		...
		if(Fillecal){
                        //cout<<"event="<<i<<"theta_gen="<<theta_gen<<endl;
		        hgen_ThetaP_cut->Fill(theta_gen,p_gen/1e3,rate);
			...
			if(procID_flux<10){
			 ...
			 hhit_Egem1_procID_other->Fill(log10(trackE_flux),rate);
					...
					process_tree_solid_ec(tree_solid_ec,tree_solid_ec_ps,Eend_ec_sum,Eend_ec_ps_sum,Eend_ec,Eend_ec_ps);
		                        hgen_ThetaEdep_cut->Fill(theta_gen,Eend_ec_sum,rate);
					if(Eend_ec_ps_sum>0){
