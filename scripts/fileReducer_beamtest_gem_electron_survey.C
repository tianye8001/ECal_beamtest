...
int fileReducer_beamtest_gem_electron_survey(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem1_Etotalcut_electron_40uA.root",the_filename), "RECREATE");
...
//	outTree->Branch("Cer_n",      &Cer_n,   "Cer_n/I"    ); //total hits on the virtual plane of GEM00 
	...
	double Ec=0;
	double trigger_ec=0;
	int hit_id=-1,pid_max=0,mpid_max=0,mvz_max=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		//	else if(flux_id->at(j)==30) hit_id=16; //
			...
			//if(flux_tid->at(j)==1 ){
				...
				//cout<<"event="<<i<<"mpid="<<flux_mpid->at(j)<<"tid="<<flux_tid->at(j)<<"pid="<<flux_pid->at(j)<<"ver_z="<<flux_avg_z->at(j)<<endl;
				...
				if (E >= edep_6p1_max){
					...
					pz_max=flux_pz->at(j);
                                        ...
                                        mvz_max = flux_mvz->at(j);
                                //cout<<"event="<<i<<"htid="<<hit_id<<"pid="<<flux_pid->at(j)<<"p_max="<<p_max<<"pf_hit="<<hit_pf<<endl;
                                 }
                               // cout<<"event="<<i<<"htid="<<hit_id<<"mpid="<<flux_mpid->at(j)<<"p_max="<<p_max<<"pf_hit="<<hit_pf<<"edep_6p1_max="<<edep_6p1_max<<endl;
		...
		int ntrigpmts_hgc=0;
		int photon_mtid=0;
              if(Is_prime==1 && pid_max==11 /*&&  N_preshower<=1*/ && edep_6p1_max>1){
              //  cout<<"event="<<i<<"pid_max="<<pid_max<<"E_max="<<edep_6p1_max<<"mpid_max="<<mpid_max<<"mvz_max="<<mvz_max<<endl;
