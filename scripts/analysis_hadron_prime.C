...
int analysis_hadron_prime(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_Nphe_tid1.root",the_filename);
					...
					if(hit_preshowerN>0){
						for (int j=0;j<solid_ec_ps_hitn->size();j++){
							...
							for (int j=0;j<solid_ec_hitn->size();j++) {
								...
								if(trigger_ec==1 && Eend_ec_ps_sum>0 && Eend_ec_sum>0){
