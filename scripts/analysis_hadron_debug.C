...
int analysis_hadron_debug(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
...
sprintf(outputfile_name, "%s_output_debug.root",the_filename);
...
// 	htime_photon[1]=new TH1F("time_photon_1","time",400,0,1000);	
	...
	TH1F *hhit_E[n],*hhit_E_mip[n],*hhit_E_photonele[n],*hhit_E_ele[n],*hhit_Edep[n],*hhit_p[n],*hhit_Ek[n],*hhit_Ec[n],*hhit_E_cut[n],*hhit_p_cut[n];		  	
	for(int i=0;i<n;i++){
	  char hstname[100];
...
// 		  if (hit_id==-1) {/cout << flux_id->at(j) << " " << flux_avg_z->at(j) << endl;
		  ...
		  double Ek=hit_p/Mp;
                  ...
                  if(E<0.07 && flux_tid->at(j) ==1){
		  cout<<"event="<<i<<"flux="<<j<<"   "<<"E="<<E<<"hit_ID="<<flux_id->at(j)<<"hit_p="<<hit_p<<"hit_pid="<<flux_pid->at(j)<<"tid="<<flux_tid->at(j)<<endl; 
                  }
		  if (flux_tid->at(j) ==1){
		  ...
		  //cout<<"event="<<i<<"flux="<<j<<"   "<<"E="<<E<<"hit_ID="<<flux_id->at(j)<<"hit_p="<<hit_p<<"hit_pid="<<flux_pid->at(j)<<"tid="<<flux_tid->at(j)<<endl; 
                    //cout<<"event="<<i<<"flux_j="<<j<<"hit_id="<<hit_id<<"PID="<<flux_pid->at(j)<<endl;
                    break;
		  }
