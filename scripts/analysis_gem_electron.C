...
int analysis_gem_electron(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_gem_electronE_pzcut_debug.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		tree_solid_gem->GetEntry(i);
			...
			mpid_gem=0;
			pid_gem=0;
			//process_tree_solid_gem(tree_solid_gem,totEdep_gem1tot,totEdep_gem2tot,totEdep_gem1,totEdep_gem2,procID_gem,mpid_gem,pid_gem);
		...
		if(Fillecal){
			...
			if(true){
				...
				for(int sensor_id=0;sensor_id<ch_hgc;sensor_id++){
					...
					double Edep_sc1=0, Edep_sc2=0,Edep_sc3=0, Edep_spd1=0, Edep_spd2=0;

					process_tree_solid_spd_simple(tree_solid_spd,Edep_sc1,Edep_sc2,Edep_sc3,Edep_spd1,Edep_spd2);
					//if(Edep_sc1>0){ 
