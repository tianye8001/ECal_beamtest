...
int analysis_gem_wrong(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_gem_gammaE_debug.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
			...
			if (hit_id==4) {//gem1
				...
				if(flux_pid->at(j)==22 && flux_pz->at(j)>0){  
					...
					pid_flux=flux_pid->at(j);
                                       if(hit_vz>-5 && hit_vz<5){
                                        cout<<"even="<<i<<"flux_j="<<j<<"vz="<<hit_vz<<"vr="<<hit_vr<<endl;
                                       }
		...
		if(Fillecal){
                                       if(hit_vz_max>-5 && hit_vz_max<5){
                                        cout<<"even="<<i<<"vz_max="<<hit_vz_max<<"vr_max="<<hit_vr_max<<endl;
                                       }
