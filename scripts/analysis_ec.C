...
int analysis_ec(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_ec_electronE_pzcut.root",the_filename);
...
//	TH3F *vertexXYZ_gem2_hist=new TH3F("vertexXYZ_gem2_hist","vertex;X (cm);Y(cm);Z (cm)",360,-20,160,360,-20,160,1020,-20,1000);	
	...
	int mpid_max=0,mpid_max_gem2=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		//			cout<<"event="<<i<<"index="<<j<<"hit_ID="<<hit_id<<"mpid_max="<<mpid_max<<"vz="<<hit_vz_max<<"vx="<<hit_vx_max<<"vy="<<hit_vy_max<<endl;
					Edepsc1 += Edep;
					//procID_flux=flux_procID->at(j);
