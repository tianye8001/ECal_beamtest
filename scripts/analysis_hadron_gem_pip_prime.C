...
int analysis_hadron_gem_pip_prime(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_ecback_electronE_pzcut_prime.root",the_filename);
...
//	Double_t totEdep_gem1=0;
//	Double_t totEdep_gem2=0;
	int procID_gem=0;
	int mpid_gem=0;
	int pid_gem=0;
	int procID_flux=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if (hit_id==3) {//gem1
				...
				if(abs(flux_pid->at(j))==211 /*&& flux_mpid->at(j)==22*/  && flux_pz->at(j)>0){  
			...
			if (hit_id==5) {//gem2
				//if(flux_pid->at(j)==22 && flux_pz->at(j)>0){
				if(flux_pid->at(j)==211 && flux_pz->at(j)>0){
		...
		if(Fillgem){
			...
			hgen_ThetaPhiP_gem2->Fill(hit_th_max_gem2,hit_phi_max_gem2,edep_6p1_max_gem2,rate);                  	        if(edep_6p1_max_gem2>0){		
