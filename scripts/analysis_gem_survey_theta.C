...
int analysis_gem_survey_theta(string inputfile_name,string runmode="trigger",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_preshower_theta_rate.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if (hit_id==4 /*&& flux_tid->at(j)==1*/) {//gem1
			...
			if (hit_id==2 /*&& flux_tid->at(j)==1*/) {//EC front 
				if(flux_pz->at(j)>0){
					if (hit_p >= edep_6p1_max){
