...
void plot_fig11(){
	...
	TH1F *ShowerT_hist = (TH1F*)gROOT->FindObject("shower_top");
        tree_SC1_electron->Draw("Shower_r>>shower_sum(8000,0,8000)","rate*(Shower_r>0)","goff");
	TH1F *ShowerSum_hist = (TH1F*)gROOT->FindObject("shower_sum");
        tree_pim->Draw("Shower_r>>shower_sum_pim(8000,0,8000)","rate*(Shower_r>0)","goff");
	TH1F *ShowerSum_pim_hist = (TH1F*)gROOT->FindObject("shower_sum_pim");
        tree_BT->Draw("Shower_r>>shower_sum_BT(8000,0,8000)","rate*(Shower_r>0)","goff");
	TH1F *ShowerSum_BT_hist = (TH1F*)gROOT->FindObject("shower_sum_BT");
        tree_pi0->Draw("Shower_r>>shower_sum_pi0(8000,0,8000)","rate*(Shower_r>0)","goff");
	TH1F *ShowerSum_pi0_hist = (TH1F*)gROOT->FindObject("shower_sum_pi0");
        tree_eAll->Draw("Shower_r>>shower_sum_eAll(8000,0,8000)","rate*(Shower_r>0)","goff");
	...
	TH1F *ShowerSum_data_hist = (TH1F*)gROOT->FindObject("shower_sum_data");
        ...
        tree_data->Draw("0.1*(Shower_r)>>shower_sum_data(8000,0,8000)","0.18011240*(TrigType==8&& (Shower_r)>0)","goff");//4*(pow(2,6)+1)/1428.92235 
	TH1F *ShowerSum_data_hist = (TH1F*)gROOT->FindObject("shower_sum_data");
        //tree_data_45uA->Draw("0.069*(Shower_r)>>shower_sum_data_45uA(8000,0,8000)","5.91*(TrigType==253 && (Shower_r)>0) ","goff");//5.91*2.25
        tree_data_45uA->Draw("0.069*(Shower_r)>>shower_sum_data_45uA(8000,0,8000)","7.385*(TrigType==253 && (Shower_t+Shower_r+Shower_l)>0) ","goff");//320ns 4./(1692515*320.0e-9)
	...
	TH1F *ShowerSum_data_45uA_hist = (TH1F*)gROOT->FindObject("shower_sum_data_45uA");
        tree_data_10uA->Draw("0.1*(Shower_r)>>shower_sum_data_10uA(8000,0,8000)","0.013836*(TrigType==8 && (Shower_r)>0) ","goff");//PS4==0; T=289.09728 //40uA
	TH1F *ShowerSum_data_10uA_hist = (TH1F*)gROOT->FindObject("shower_sum_data_10uA");
        //tree_data_accidental->Draw("0.1*(Shower_r)>>shower_sum_data_acc(8000,0,8000)","4.7292324*(Shower_r>0) ","goff");//10-20 bins: T=(40e-9*Ntotal_with (Shower_t+Shower_r+Shower_l)>0) //40uA
        tree_data_accidental->Draw("0.11*(Shower_r)>>shower_sum_data_acc(8000,0,8000)","10.7*((Shower_t+Shower_r+Shower_l)>0) ","goff");//10-20 bins: T=(40e-9*Ntotal_with (Shower_t+Shower_r+Shower_l)>0) //40uA
        //tree_data_accidental->Draw("0.1*(Shower_r)>>shower_sum_data_acc(8000,0,8000)","10.7*(Shower_r>0) ","goff");//10-20 bins: T=(40e-9*Ntotal_with (Shower_t+Shower_r+Shower_l)>0) //40uA
	...
	gPad->SetLogy();
        ...
        ShowerSum_pim_hist->SetTitle("Shower_r ");
        ShowerSum_pim_hist->GetXaxis()->SetTitle("Shower_r Edep (MeV)");
      ...
      //  ShowerSum_BT_hist->SetLineWidth(2);
        ...
        ShowerSum_pim_clone_hist->SetTitle("Shower_r ");
        ShowerSum_pim_clone_hist->GetXaxis()->SetTitle("Shower_r Edep (MeV)");
