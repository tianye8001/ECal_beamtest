...
//	TH1F *shower_beam_dum_hist = (TH1F*)gROOT->FindObject("shower_beam_dum");
	...
	TH1F *shower_eAll_hist = (TH1F*)gROOT->FindObject("shower_eAll");
        tree_eAll->Draw("Npesum>>shower_beam(30,0.5,30.5)","rate*(abs(GEM00_x)<2 && abs(GEM00_y)<2)","goff");
	TH1F *shower_beam_hist = (TH1F*)gROOT->FindObject("shower_beam");
	...
	TH1F *shower_all_hist = (TH1F*)gROOT->FindObject("shower_all");
       /* tree_beam_dum->Draw("ShowerSum>>shower_beam_dum(200,0,1200)","rate*(ShowerSum>600 && Shower_t>5.8)","goff");
        TH1F *shower_beam_dum_hist = (TH1F*)gROOT->FindObject("shower_beam_dum");*/
        tree_eAll->Draw("PreShtheta>>theta_pip(900,0.5,90.5)","rate*(abs(GEM00_x)<2 && abs(GEM10_x)<2 && ShowerSum>900)","goff");
	TH1F *theta_pip_hist = (TH1F*)gROOT->FindObject("theta_pip");
        tree_eAll->Draw("PreShtheta>>theta_pim(900,0.5,90.5)","rate*(abs(GEM00_x)<2 && abs(GEM00_y)<2 )","goff");
	TH1F *theta_pim_hist = (TH1F*)gROOT->FindObject("theta_pim");
        tree_eAll->Draw("PreShtheta>>theta_pi0(900,0.5,90.5)","rate*(ShowerSum>800)","goff");
	TH1F *theta_pi0_hist = (TH1F*)gROOT->FindObject("theta_pi0");
        tree_eAll->Draw("PreShtheta>>theta_eAll(900,0.5,90.5)","rate*(ShowerSum>1100)","goff");
	TH1F *theta_eAll_hist = (TH1F*)gROOT->FindObject("theta_eAll");
        tree_eAll->Draw("PreShtheta>>theta_beam(900,0.5,90.5)","rate*(abs(GEM00_x)<2 && abs(GEM00_y)<2)","goff");
	TH1F *theta_beam_hist = (TH1F*)gROOT->FindObject("theta_beam");
        tree_eAll->Draw("PreShtheta>>theta_all(900,0.5,90.5)","rate*(PreShtheta>0)","goff");
	...
	gPad->SetGridy();
	gPad->SetLogy();
        shower_beam_hist->SetLineColor(1);
        shower_beam_hist->GetXaxis()->SetTitle("Npe");
        ...
        shower_beam_hist->GetYaxis()->SetRangeUser(1e-4,2e0);
        shower_beam_hist->SetLineWidth(2);
        shower_beam_hist->Draw();
        double rate_proton= shower_beam_hist->Integral();
        /*shower_pip_hist->SetLineColor(2);
        shower_pip_hist->SetLineWidth(2);
        ...
        shower_pim_hist->SetLineWidth(2);
  ...
  TLegend *leg8 = new TLegend(0.15,0.6,0.8,0.88);
  ...
  leg8->AddEntry(shower_beam_hist,Form("abs(GEM00_x)<2 & abs(GEM00_y)<2 cuts; total rate=%f kHz",rate_proton),"l");
  ...
  leg8->Draw("text same");
	...
	gPad->SetGridy();
	gPad->SetLogy();
        theta_beam_hist->SetLineColor(1);
        theta_beam_hist->GetXaxis()->SetTitle("hit #theta_{Cer}");
        theta_beam_hist->SetTitle(" e^{-} from target hit on Cer  ");
        theta_beam_hist->GetYaxis()->SetTitle("Rate (kHz)");
        theta_beam_hist->Scale(0.001);
        theta_beam_hist->GetYaxis()->SetRangeUser(1e-4,1e1);
        theta_beam_hist->GetXaxis()->SetRangeUser(0,15);
        theta_beam_hist->SetLineWidth(1.5);
        theta_beam_hist->Draw();
        double rate_proton_theta= theta_beam_hist->Integral();
  ...
  TLegend *leg18 = new TLegend(0.15,0.6,0.8,0.88);
 // leg18->AddEntry(theta_all_hist,Form("#theta_{Cer} >0 cut; total rate=%f kHz",rate_all_theta),"l");
  leg18->AddEntry(theta_beam_hist,Form("abs(GEM00_x)<2 & abs(GEM00_y)<2 cuts; total rate=%f kHz",rate_proton_theta),"l");
 // leg18->AddEntry(theta_eAll_hist,Form("Shower Edep >1100 MeV cut; rate=%f kHz",rate_eAll_theta),"l");
 // leg18->AddEntry(theta_pim_hist,Form("abs(GEM00_x)<2 && abs(GEM00_y)<2; total rate=%f kHz",rate_pim_theta),"l");
 // leg18->AddEntry(theta_pi0_hist,Form("Shower Edep >800 MeV cut; total rate=%f kHz",rate_pi0_theta),"l");
 // leg18->AddEntry(theta_pip_hist,Form("#splitline{abs(GEM00_x)<2 && abs(GEM10_x)<2}{ && ShowerSum>800; total rate=%f kHz}",rate_pip_theta),"l");
  //leg8->AddEntry(shower_beam_dum_hist,Form("BeamOnTarget dummy; total rate=%f kHz",rate_beam),"l");
