...
int fileReducer_beamtest_gem_e(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_preshower_Etotalcut_esum.root",the_filename), "RECREATE");
	//TFile* outFile = new TFile(Form("%s_reduce_tree_rate_CC_eEcut.root",the_filename), "RECREATE");
//	TFile* outFile = new TFile("reduce_tree_rate_Nphe_Certest.root", "RECREATE");
...
//	ofstream outfile1(Form("%s_output.csv",the_filename));
	...
	float rateRad = 0;
	float Npesum=0;
       // float hit_Cer[MAX_CHANNEL];
	int ecN=0;
	float PreShP,PreShP_e,PreShPx,PreShPy,PreShPz,PreShSum, PreSh_l, PreSh_r, PreSh_t,PreShtheta,PreShtheta_e;
	//  float ShP,ShPx,ShPy,ShPz;
	float ShowerSum, Shower_l, Shower_r, Shower_t;
	float LASPD_Eendsum, SPD_Eendsum, SC0_Eendsum, SC1_Eendsum, SC2_Eendsum,SC3_Eendsum;
	float LASPD_Eend, SPD_Eend, SC0_Eend, SC1_Eend, SC2_Eend,SC3_Eend;
	float SC0_P;
	float SC1_P;
	float SC2_P;
	float SC3_P;
	...
	outTree->Branch("PreShtheta_e",      &PreShtheta_e,   "PreShtheta_e/F"    ); //momentum pz hit on the virtual plane of presh
	...
	outTree->Branch("SC2_P",      &SC2_P,   "SC2_P/F"    ); //momentum hit on the virtual plane of SC1
	outTree->Branch("SC2_Eendsum",      &SC2_Eendsum,   "SC2_Eendsum/F"    ); //Deposit energy in the SC1
	outTree->Branch("SC2_Eend",      &SC2_Eend,   "SC2_Eend/F"    ); //prime particle deposit energy in the SC1
	outTree->Branch("SC3_P",      &SC3_P,   "SC3_P/F"    ); //momentum hit on the virtual plane of SC2
	outTree->Branch("SC3_Eendsum",      &SC3_Eendsum,   "SC3_Eendsum/F"    ); //Deposit energy in the SC2
	outTree->Branch("SC3_Eend",      &SC3_Eend,   "SC3_Eend/F"    ); //prime particle deposit energy in the SC2
...
//	outTree->Branch("Cer_n",      &Cer_n,   "Cer_n/I"    ); //total hits on the virtual plane of GEM00 
	...
	double px_max=0,py_max=0,pz_max=0,px_max_e=0,py_max_e=0,pz_max_e=0,p_max=0,p_max_e=0,theta_max=0,theta_max_e=0;
	double SC0_p_max=0;
	double SC1_p_max=0;
	double SC2_p_max=0;
	double SC3_p_max=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		if (evgen==2){
			...
			rate=5e-6/1.6e-19/event_actual;  //beamOnTarget  file I= 5 uA	 	  
			//rate=60e-6/1.6e-19/event_actual;  //beamOnTarget  file I= 5 uA	 	  
		}
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			else if(flux_id->at(j)==4) hit_id=3; // SC3 front 
			...
			else if(flux_id->at(j)==11) hit_id=17; // SC2 front 
			else if(flux_id->at(j)==13) hit_id=10; // 
			else if(flux_id->at(j)==20) hit_id=11; // 
			else if(flux_id->at(j)==21) hit_id=12; // 
			...
			//if(flux_tid->at(j)==1 ){
				//cout<<"event="<<i<<"hit_id="<<hit_id<<"tid="<<flux_tid->at(j)<<"pid="<<flux_pid->at(j)<<endl;
				//if(hit_id==1 && abs(flux_pid->at(j))==22){
				if(hit_id==2 && flux_mtid->at(j)==1 && flux_mpid->at(j)==22 && flux_tid->at(j)<=3){
				//if(hit_id==3 /*&& abs(flux_pid->at(j))==11*/ && abs(flux_avg_lx->at(j)/1e1)<2.5 && abs(flux_avg_ly->at(j)/1e1)<5){
                                //cout<<"event="<<i<<"flux_index="<<j<<"htid="<<hit_id<<"pid="<<flux_pid->at(j)<<"mpid="<<flux_mpid->at(j)<<"mtid="<<flux_mtid->at(j)<<"tid="<<flux_tid->at(j)<<"pf_hit="<<hit_pf<<endl;
                                Is_prime=1;
                                if(flux_tid->at(j)==3 && flux_pid->at(j)==-11){
					edep_6p1_max=E;
					p_max=hit_pf;
					px_max=flux_px->at(j);
					py_max=flux_py->at(j);
					pz_max=flux_pz->at(j);
                                ...
                                if(flux_tid->at(j)==2 && flux_pid->at(j)==11){
					edep_6p1_max_e=E;
					p_max_e=hit_pf;
					px_max_e=flux_px->at(j);
					py_max_e=flux_py->at(j);
					pz_max_e=flux_pz->at(j);
                                        theta_max_e = hit_th;
                                 }
                                //cout<<"event="<<i<<"htid="<<hit_id<<"pid="<<flux_pid->at(j)<<"p_max="<<p_max<<"pf_hit="<<hit_pf<<endl;
                                //cout<<"event="<<i<<"htid="<<hit_id<<"pid="<<flux_pid->at(j)<<"p_max="<<p_max<<"pf_hit="<<hit_pf<<"edep_6p1_max="<<edep_6p1_max<<endl;
				} //else Is_prime=0;
				if(hit_id==0 ){
					SC0_p_max=hit_pf;
				}
			...
			//cout<<"event="<<i<<"j="<<j<<" CC_x="<<flux_avg_lx->at(j)*0.1<<"CC_y="<<flux_avg_ly->at(j)*0.1<<endl;
				Is_trig=1;
			}
			//cout<<"event="<<i<<"j="<<j<<" GEM00_n="<<GEM00_n<<"GEM10_n="<<GEM10_n<<endl;
		}	// end of flux		
                // process gem
             if(Is_prime==1 && p_max>0 && p_max_e>0/*&& edep_6p1_max<0.06&& p_max>200*/){
		...
		int pid_gem=0;
			process_tree_solid_gem(tree_solid_gem,totEdep_gem1_gas,totEdep_gem2_gas,totEdep_gem1tot,totEdep_gem2tot, procID_gem,mpid_gem,pid_gem);
                ...
                //cout<<"event="<<i<<"GEM00_Edep="<<GEM00_Edep<<"GEM10_Edep="<<GEM10_Edep<<endl;
		// process ec
		...
		process_tree_solid_ec(tree_solid_ec,tree_solid_ec_ps,Eend_ec_sum,Eend_ec_ps_sum,Eend_ec,Eend_ec_ps);
         //      if(Eend_ec_sum>40){
//		cout<<"event="<<i<<"pid="<<pid_max<<"p_max="<<p_max<<"Eend_ec_sum="<<Eend_ec_sum<<endl;
  //             }
		//if(Eend_ec_ps_sum>0){ 
		...
		//PreShP=p_max;
		PreShP=edep_6p1_max;
		PreShP_e=edep_6p1_max_e;
		PreShPx=px_max;
		PreShPy=py_max;
		PreShPz=pz_max;
		PreShtheta=theta_max;
		PreShtheta_e=theta_max_e;
		//	writeFlag = true;
		//cout<<"event="<<i<<"  pid_gen="<<pid_gen<<endl;
		//if(Eend_ec_ps_sum>40){
		...
		if(SC1_P>0){
			SC1_Eend = Edep_sc2;
                ...
                //SC2
		SC2_P=SC2_p_max;
		SC2_Eendsum = Edep_sc3;
		if(SC2_P>0){
			SC2_Eend = Edep_sc3;
		}
                //SC3
		SC3_P=SC3_p_max;
		SC3_Eendsum = Edep_sc4;
		if(SC3_P>0){
			SC3_Eend = Edep_sc4;
		}
		//SPD
		...
		for(int sensor_id=0;sensor_id<ch_hgc;sensor_id++) {	
			if (hit_hgc[sensor_id]>0){
				...
				//Cer_hit[sensor_id] = hit_hgc[sensor_id];
				//cout<<"sensor_id="<<sensor_id<<"hit_hgc="<<hit_hgc[sensor_id]<<"ntrigsecs_hgc="<<ntrigsecs_hgc<<"ntrigpmts_hgc="<<ntrigpmts_hgc<<endl;
		...
		Npesum=npe_hgc_total;
               // Cer_n=ntrigsecs_hgc;
                //cout<<"event="<<i<<"ntrigsecs_hgc="<<ntrigsecs_hgc<<"ntrigpmts_hgc="<<ntrigpmts_hgc<<"Npesum="<<npe_hgc_total<<endl;
              //}
		//GEM00
		//if (Is_prime==1){
                //}
	...
	//	}
         ...
         // if(Is_prime==1 && pid_max==11 && mpid_max==22/*&& edep_6p1_max<0.06&& p_max>200*/){
          //       PreShP_e=edep_6p1_max;     
          //  }
		outTree->Fill();
	} //end loop
	//do outputs
