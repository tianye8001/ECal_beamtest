...
int analysis_eAll(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_Nphe_finetime_Q2cut_10MeV.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		if(userVar005->at(0)>0.01){ //Q2cut for small angle 
			...
			for (Int_t j=0;j<flux_hitn->size();j++) {
				...
				else if(flux_id->at(j)==9) hit_id=9; // spd 2	  
