...
/*	TTree *tree_beam_pion = (TTree*) f9->Get("T");
	...
	/*	int ndata = tree_eAll->Draw("(GEM00_x-GEM10_x):(GEM00_y-GEM10_y)",
		...
		*/
        //tree_pip->Draw("SC1_Eendsum>>SC1_pip(100,0,50)","rate*(SC1_Eendsum>0 && SC0_Eendsum>0.88)","goff");
        tree_pip->Draw("SC_A_Eendsum>>SC0_pip(100,0,10)","rate*( SC_A_Eendsum>0.99 )","goff");
	TH1F *SC0_pip_hist = (TH1F*)gROOT->FindObject("SC0_pip");
        tree_pim->Draw("SC_A_Eendsum>>SC0_pim(100,0,10)","rate*(SC_A_Eendsum>0.99 )","goff");
	TH1F *SC0_pim_hist = (TH1F*)gROOT->FindObject("SC0_pim");
        tree_pi0->Draw("SC_A_Eendsum>>SC0_pi0(100,0,10)","rate*(SC_A_Eendsum>0.99 )","goff");
	TH1F *SC0_pi0_hist = (TH1F*)gROOT->FindObject("SC0_pi0");
        tree_beam->Draw("SC_A_Eendsum>>SC0_beam(100,0,10)","rate*(SC_A_Eendsum>0.99 )","goff");
	TH1F *SC0_beam_hist = (TH1F*)gROOT->FindObject("SC0_beam");
        tree_beam_pb->Draw("SC_A_Eendsum>>SC0_beam_pb(100,0,10)","rate*(SC_A_Eendsum>0.99 )","goff");
	TH1F *SC0_beam_pb_hist = (TH1F*)gROOT->FindObject("SC0_beam_pb");
        tree_eAll->Draw("SC_A_Eendsum>>SC0_eAll(100,0,10)","rate*(SC_A_Eendsum>0.99 )","goff");
	TH1F *SC0_eAll_hist = (TH1F*)gROOT->FindObject("SC0_eAll");
    
       tree_eAll->Draw("ShowerSum>>shower_eAll(1000,0,2000)","rate*(ShowerSum>133 )","goff");
	TH1F *shower_eAll_hist = (TH1F*)gROOT->FindObject("shower_eAll");
        tree_pip->Draw("ShowerSum>>shower_pip(1000,0,2000)","rate*(ShowerSum>133 )","goff");
	TH1F *shower_pip_hist = (TH1F*)gROOT->FindObject("shower_pip");
        tree_pim->Draw("ShowerSum>>shower_pim(1000,0,2000)","rate*(ShowerSum>133)","goff");
	TH1F *shower_pim_hist = (TH1F*)gROOT->FindObject("shower_pim");
        tree_pi0->Draw("ShowerSum>>shower_pi0(1000,0,2000)","rate*(ShowerSum>133)","goff");
	TH1F *shower_pi0_hist = (TH1F*)gROOT->FindObject("shower_pi0");
        tree_beam->Draw("ShowerSum>>shower_beam(1000,0,2000)","rate*(ShowerSum>200)","goff");
	TH1F *shower_beam_hist = (TH1F*)gROOT->FindObject("shower_beam");
        tree_beam_pb->Draw("ShowerSum>>shower_beam_pb(100,0,50)","rate*(ShowerSum>133)","goff");
	TH1F *shower_beam_bp_hist = (TH1F*)gROOT->FindObject("shower_beam_pb");

        tree_pip->Draw("SC_D_Eendsum>>SC1_pip(100,0,10)","rate*( SC_D_Eendsum>1.75 )","goff");
	TH1F *SC1_pip_hist = (TH1F*)gROOT->FindObject("SC1_pip");
        tree_pim->Draw("SC_D_Eendsum>>SC1_pim(100,0,10)","rate*(SC_D_Eendsum>1.75 )","goff");
	TH1F *SC1_pim_hist = (TH1F*)gROOT->FindObject("SC1_pim");
        tree_pi0->Draw("SC_D_Eendsum>>SC1_pi0(100,0,10)","rate*(SC_D_Eendsum>1.75 )","goff");
	TH1F *SC1_pi0_hist = (TH1F*)gROOT->FindObject("SC1_pi0");
        tree_beam->Draw("SC_D_Eendsum>>SC1_beam(100,0,10)","rate*(SC_D_Eendsum>1.75 )","goff");
	TH1F *SC1_beam_hist = (TH1F*)gROOT->FindObject("SC1_beam");
        tree_beam_pb->Draw("SC_D_Eendsum>>SC1_beam_pb(100,0,10)","rate*(SC_D_Eendsum>1.75 )","goff");
	TH1F *SC1_beam_pb_hist = (TH1F*)gROOT->FindObject("SC1_beam_pb");
        tree_eAll->Draw("SC_D_Eendsum>>SC1_eAll(100,0,10)","rate*(SC_D_Eendsum>1.75 )","goff");
	TH1F *SC1_eAll_hist = (TH1F*)gROOT->FindObject("SC1_eAll");

        tree_pip->Draw("SC_B_Eendsum>>SC2_pip(100,0,10)","rate*( SC_B_Eendsum>0.99 )","goff");
	TH1F *SC2_pip_hist = (TH1F*)gROOT->FindObject("SC2_pip");
        tree_pim->Draw("SC_B_Eendsum>>SC2_pim(100,0,10)","rate*(SC_B_Eendsum>0.99 )","goff");
	TH1F *SC2_pim_hist = (TH1F*)gROOT->FindObject("SC2_pim");
        tree_pi0->Draw("SC_B_Eendsum>>SC2_pi0(100,0,10)","rate*(SC_B_Eendsum>0.99 )","goff");
	TH1F *SC2_pi0_hist = (TH1F*)gROOT->FindObject("SC2_pi0");
        tree_beam->Draw("SC_B_Eendsum>>SC2_beam(100,0,10)","rate*(SC_B_Eendsum>0.99 )","goff");
	TH1F *SC2_beam_hist = (TH1F*)gROOT->FindObject("SC2_beam");
        tree_beam_pb->Draw("SC_B_Eendsum>>SC2_beam_pb(100,0,10)","rate*(SC_B_Eendsum>0.99 )","goff");
	TH1F *SC2_beam_pb_hist = (TH1F*)gROOT->FindObject("SC2_beam_pb");
        tree_eAll->Draw("SC_B_Eendsum>>SC2_eAll(100,0,10)","rate*(SC_B_Eendsum>0.99 )","goff");
 ...
 // leg8->AddEntry(shower_beam_pion_hist,Form("hit on prelead from BeamOnTarget ; total rate=%f kHz",rate_beam_pion),"l");
  ...
  leg8->Draw("text same");
	...
	gPad->SetLogy();
        ...
        SC2_beam_hist->GetXaxis()->SetTitle("SC_B Eend (MeV)");
  ...
  leg18->Draw("text same");
	...
	gPad->SetLogy();
        ...
        SC0_beam_hist->GetXaxis()->SetTitle("SC_A Eend (MeV)");
        SC0_beam_hist->SetTitle("10.6 GeV 5 #muA C-foil target 7 degrees ");
        SC0_beam_hist->GetYaxis()->SetTitle("Rate (kHz)");
        SC0_beam_hist->Scale(0.001);
        SC0_beam_hist->GetYaxis()->SetRangeUser(1e-4,1e7);
        SC0_beam_hist->Draw();
        double rate_proton_SC0= SC0_beam_hist->Integral();
        SC0_pip_hist->SetLineColor(2);
        SC0_pip_hist->Scale(0.001);
        SC0_pip_hist->Draw("same HIST");
        double rate_pip_SC0= SC0_pip_hist->Integral();
        SC0_pim_hist->SetLineColor(1);
        SC0_pim_hist->Scale(0.001);
        SC0_pim_hist->Draw("same HIST");
        double rate_pim_SC0= SC0_pim_hist->Integral();
        SC0_pi0_hist->SetLineColor(8);
        SC0_pi0_hist->Scale(0.001);
        SC0_pi0_hist->Draw("same HIST");
        double rate_pi0_SC0= SC0_pi0_hist->Integral();
        SC0_beam_pb_hist->SetLineColor(28);
        SC0_beam_pb_hist->Scale(0.001);
        SC0_beam_pb_hist->Draw("same HIST");
        double rate_beam_pb_SC0= SC0_beam_pb_hist->Integral();
        SC0_eAll_hist->SetLineColor(kOrange-3);
        SC0_eAll_hist->Scale(0.001);
        SC0_eAll_hist->Draw("same HIST");
        double rate_eAll_SC0= SC0_eAll_hist->Integral();
        SC0_pip_hist->Draw("same HIST");
        SC0_pim_hist->Draw("same HIST");
  TLegend *leg1 = new TLegend(0.15,0.6,0.6,0.88);
  leg1->AddEntry(SC0_pim_hist,Form("from bggen #pi^{-} ; total rate=%f kHz",rate_pim_SC0),"l");
  leg1->AddEntry(SC0_pip_hist,Form("from bggen #pi^{+} ; total rate=%f kHz",rate_pip_SC0),"l");
  leg1->AddEntry(SC0_pi0_hist,Form("from bggen #pi^{0} ; total rate=%f kHz",rate_pi0_SC0),"l");
  leg1->AddEntry(SC0_eAll_hist,Form("from eAll ; total rate=%f kHz",rate_eAll_SC0),"l");
  leg1->AddEntry(SC0_beam_hist,Form("from BeamOnTarget ; total rate=%f kHz",rate_proton_SC0),"l");
  leg1->AddEntry(SC0_beam_pb_hist,Form("from BeamOnTarget+ECal shielding; total rate=%f kHz",rate_beam_pb_SC0),"l");
  ...
  leg1->Draw("text same");
	...
	gPad->SetLogy();
        ...
        SC1_beam_hist->GetXaxis()->SetTitle("SC_D Eend (MeV)");
        SC1_beam_hist->SetTitle("10.6 GeV 5 #muA C-foil target 7 degrees ");
        SC1_beam_hist->GetYaxis()->SetTitle("Rate (kHz)");
        SC1_beam_hist->Scale(0.001);
        SC1_beam_hist->GetYaxis()->SetRangeUser(1e-4,1e7);
        SC1_beam_hist->Draw();
        double rate_proton_SC1= SC1_beam_hist->Integral();
        SC1_pip_hist->SetLineColor(2);
        SC1_pip_hist->Scale(0.001);
        SC1_pip_hist->Draw("same HIST");
        double rate_pip_SC1= SC1_pip_hist->Integral();
        SC1_pim_hist->SetLineColor(1);
        SC1_pim_hist->Scale(0.001);
        SC1_pim_hist->Draw("same HIST");
        double rate_pim_SC1= SC1_pim_hist->Integral();
        SC1_pi0_hist->SetLineColor(8);
        SC1_pi0_hist->Scale(0.001);
        SC1_pi0_hist->Draw("same HIST");
        double rate_pi0_SC1= SC1_pi0_hist->Integral();
        SC1_beam_pb_hist->SetLineColor(28);
        SC1_beam_pb_hist->Scale(0.001);
        SC1_beam_pb_hist->Draw("same HIST");
        double rate_beam_pb_SC1= SC1_beam_pb_hist->Integral();
        SC1_eAll_hist->SetLineColor(kOrange-3);
        SC1_eAll_hist->Scale(0.001);
        SC1_eAll_hist->Draw("same HIST");
        double rate_eAll_SC1= SC1_eAll_hist->Integral();
        SC1_pip_hist->Draw("same HIST");
        SC1_pim_hist->Draw("same HIST");
  TLegend *leg8 = new TLegend(0.15,0.6,0.6,0.88);
  leg8->AddEntry(SC1_pim_hist,Form("from bggen #pi^{-} ; total rate=%f kHz",rate_pim_SC1),"l");
  leg8->AddEntry(SC1_pip_hist,Form("from bggen #pi^{+} ; total rate=%f kHz",rate_pip_SC1),"l");
  leg8->AddEntry(SC1_pi0_hist,Form("from bggen #pi^{0} ; total rate=%f kHz",rate_pi0_SC1),"l");
  leg8->AddEntry(SC1_eAll_hist,Form("from eAll ; total rate=%f kHz",rate_eAll_SC1),"l");
  leg8->AddEntry(SC1_beam_hist,Form("from BeamOnTarget ; total rate=%f kHz",rate_proton_SC1),"l");
  leg8->AddEntry(SC1_beam_pb_hist,Form("from BeamOnTarget+ECal shielding; total rate=%f kHz",rate_beam_pb_SC1),"l");
  leg8->SetTextSize(0.03);
  leg8->SetBorderSize(0);
  leg8->SetFillColor(0);
  leg8->Draw("text same");
