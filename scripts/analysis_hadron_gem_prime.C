...
int analysis_hadron_gem_prime(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_gem_gammaE_prime.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		if(Fillecal){
			...
			if(totEdep_gem1>0 /*&& pid_gem==22*/){
				//cout<<"event="<<i<<"pf="<<edep_6p1_max<<endl;
				//hprocid_gem1->Fill(procID_gem,rate);
				//cout<<"event="<<i<<"totE_gem="<<totEdep_gem<<"procID_gem="<<procID_gem<<"procID_flux="<<procID_flux<<"mpid_gem="<<mpid_gem<<"mpid_flux="<<mpid_flux<<"pid_flux="<<pid_flux<<"pid_gem="<<pid_gem<<endl;
				hhit_Edepgem1->Fill(log10(totEdep_gem1),rate);
				hhit_E_gem1->Fill(log10(trackE_flux),rate);
				hhit_p_vs_Edepgem1->Fill(log10(totEdep_gem1),edep_6p1_max,rate);
				//if(mpid_gem==111){
				if(mpid_flux==111){
					hhit_Edepgem1_pi0->Fill(log10(totEdep_gem1),rate);
				}else if(abs(mpid_flux)==11){
					hhit_Edepgem1_electron->Fill(log10(totEdep_gem1),rate);
				}else if(mpid_flux==22){
					hhit_Edepgem1_gamma->Fill(log10(totEdep_gem1),rate);
				}else{
					hhit_Edepgem1_other->Fill(log10(totEdep_gem1),rate);
					// cout<<"event="<<i<<"totE_gem="<<totEdep_gem<<"procID="<<procID_gem<<"procID_flux="<<procID_flux<<"mpid_gem="<<mpid_gem<<endl;
				}
			}
			if(procID_flux<10){
				hhit_Edepgem1_procID[procID_flux]->Fill(log10(totEdep_gem1),rate);
			}else{
				hhit_Edepgem1_procID_other->Fill(log10(totEdep_gem1),rate);
			}
			}//end of Fillall
