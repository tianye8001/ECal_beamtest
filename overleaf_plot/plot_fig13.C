...
void plot_fig13(){
	...
	TFile *f33 = new TFile("/lustre19/expphy/volatile/halla/solid/tianye/ecal_beamtest_hallc/18deg/GEMROOTFILE_T1020/beamtest_level1_4770_4787_nobeamtrip.root");
        ...
        TFile *f17 = new TFile("/lustre19/expphy/volatile/halla/solid/tianye/container/HallC_beamtest_beamOntarget_60uA_sangle_4SC_18deg_LD2_r20m_shielding_30x30x2.54cm3_EMonly/HallC_beamtest_beamOntarget_40uA_sangle_4SC_18deg_LD2_r20m_shielding_30x30x2.54cm3_BeamOnTargetEM_1.0001e11_3MeVfilter_reduce_tree_rate_resolution_Eend.root");
	...
	TTree *tree_BT2 = (TTree*) f17->Get("T");
	...
	TH1F *ShowerT_hist = (TH1F*)gROOT->FindObject("shower_top");
        tree_SC1_electron->Draw("ShowerSum>>shower_sum(8000,0,8000)","rate*(ShowerSum>0 )","goff");
	TH1F *ShowerSum_hist = (TH1F*)gROOT->FindObject("shower_sum");
        tree_pim->Draw("ShowerSum>>shower_sum_pim(8000,0,8000)","rate*(ShowerSum>0 )","goff");
	TH1F *ShowerSum_pim_hist = (TH1F*)gROOT->FindObject("shower_sum_pim");
        tree_BT->Draw("ShowerSum>>shower_sum_BT(8000,0,8000)","rate*(ShowerSum>0)","goff");
	TH1F *ShowerSum_BT_hist = (TH1F*)gROOT->FindObject("shower_sum_BT");
        tree_BT2->Draw("ShowerSum>>shower_sum_BT2(8000,0,8000)","rate*(ShowerSum>0)","goff");
	TH1F *ShowerSum_BT2_hist = (TH1F*)gROOT->FindObject("shower_sum_BT2");
        tree_pi0->Draw("ShowerSum>>shower_sum_pi0(8000,0,8000)","rate*(ShowerSum>0)","goff");
	TH1F *ShowerSum_pi0_hist = (TH1F*)gROOT->FindObject("shower_sum_pi0");
        tree_eAll->Draw("ShowerSum>>shower_sum_eAll(8000,0,8000)","rate*(ShowerSum>0)","goff");
	...
	TH1F *ShowerSum_data_hist = (TH1F*)gROOT->FindObject("shower_sum_data");
        ...
        //tree_data->Draw("(0.94913439*Shower_l+Shower_r+Shower_t)*0.11>>shower_sum_data(8000,0,8000)","0.18011240*(TrigType==8&& (0.94913439*Shower_l+Shower_r+Shower_t)>0)","goff");//4*(pow(2,6)+1)/1428.92235 
        tree_data->Draw("(Shower_l+Shower_r+Shower_t)*0.11>>shower_sum_data(8000,0,8000)","0.18011240*(TrigType==8&& (Shower_l+Shower_r+Shower_t)>0)","goff");//4*(pow(2,6)+1)/1428.92235 
        //tree_data->Draw("(Shower_l+Shower_r+Shower_t)*0.11>>shower_sum_data(8000,0,8000)","0.2567*(TrigType==8&& (Shower_l+Shower_r+Shower_t)>0)","goff");//4*(pow(2,6)+1)/1.01300488999999993e+03 
	TH1F *ShowerSum_data_hist = (TH1F*)gROOT->FindObject("shower_sum_data");
        //tree_data_45uA->Draw("0.11*(Shower_t+Shower_r+Shower_l)>>shower_sum_data_45uA(8000,0,8000)","5.91*(TrigType==253 && (Shower_t+Shower_r+Shower_l)>0) ","goff");//5.91*2.25
        tree_data_45uA->Draw("0.073*(Shower_t+Shower_r+Shower_l)>>shower_sum_data_45uA(8000,0,8000)","7.385*(TrigType==253 && (Shower_t+Shower_r+Shower_l)>0) ","goff");//320ns 4./(1692515*320.0e-9)
	TH1F *ShowerSum_data_45uA_hist = (TH1F*)gROOT->FindObject("shower_sum_data_45uA");
        tree_data_10uA->Draw("0.11*(Shower_t+Shower_r+Shower_l)>>shower_sum_data_10uA(8000,0,8000)","0.013836*(TrigType==8 && (Shower_t+Shower_r+Shower_l)>0) ","goff");//PS4==0; T=289.09728 //40uA
	TH1F *ShowerSum_data_10uA_hist = (TH1F*)gROOT->FindObject("shower_sum_data_10uA");
        //tree_data_accidental->Draw("0.11*(Shower_t+Shower_r+Shower_l)>>shower_sum_data_acc(8000,0,8000)","4.7292324*((Shower_t+Shower_r+Shower_l)>0) ","goff");//10-20 bins: T=(40e-9*Ntotal_with (Shower_t+Shower_r+Shower_l)>0) //40uA
       // tree_data_accidental->Draw("0.11*(Shower_t+Shower_r+Shower_l)>>shower_sum_data_acc(8000,0,8000)","380.8*((Shower_t+Shower_r+Shower_l)>0) ","goff");//10-20 bins: T=(40e-9*Ntotal_with (Shower_t+Shower_r+Shower_l)>0) //40uA
        tree_data_accidental->Draw("0.11*(Shower_t+Shower_r+Shower_l)>>shower_sum_data_acc(8000,0,8000)","10.7*((Shower_t+Shower_r+Shower_l)>0) ","goff");//10-20 bins: T=(40e-9*Ntotal_with (Shower_t+Shower_r+Shower_l)>0) //40uA
	...
	gPad->SetGridy();
	gPad->SetLogy();
        ShowerSum_pim_hist->SetLineColor(6);
        ShowerSum_pim_hist->SetTitle("ShowerSum ");
        ShowerSum_pim_hist->GetXaxis()->SetTitle("ShowerSum Edep (MeV)");
      ...
      //  ShowerSum_eAll_hist->Draw("same HIST");
        ShowerSum_BT_hist->SetLineColor(9);
      ...
      //  ShowerSum_BT_hist->SetLineWidth(2);
        ...
        ShowerSum_BT2_hist->SetLineColor(49);
        ShowerSum_BT2_hist->Rebin(rebinfac);
        //ShowerSum_BT2_hist->Draw("same HIST");
        ...
        ShowerSum_pim_clone_hist->SetTitle("ShowerSum ");
        ShowerSum_pim_clone_hist->GetXaxis()->SetTitle("ShowerSum Edep (MeV)");
        ...
        ShowerSum_pim_clone_hist->SetLineColor(2);
        ...
        ShowerSum_data_accidental_hist->SetLineColor(1);
       // ShowerSum_data_accidental_hist->Scale(3);
       ...
       //leg21->AddEntry(ShowerSum_BT2_hist,"beam test simulation BeamOnTarget EM >3MeV ","l");
        ...
        //leg21->AddEntry(ShowerSum_data_accidental_hist,"beam test Out-Of-Time 10uA*4","l");
