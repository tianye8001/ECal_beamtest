...
int fileReducer_beamtest_gem_test(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	//TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem_Etotalcut.root",the_filename), "RECREATE");
	TFile* outFile = new TFile("reduce_tree_rate_gem_noCCcentercut_badmirror.root", "RECREATE");
...
//	ofstream outfile1(Form("%s_output.csv",the_filename));
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if(hit_id==6 && abs(flux_avg_lx->at(j)*0.1)<5 && abs(flux_avg_ly->at(j)*0.1)<5){                                
