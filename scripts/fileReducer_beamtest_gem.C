...
int fileReducer_beamtest_gem(string inputfile_name,int numberOfFile=1, double event_actual=1, int evgen=1){

	char the_filename[500];
	sprintf(the_filename, "%s",inputfile_name.substr(0,inputfile_name.rfind(".")).c_str());
	TFile* outFile = new TFile(Form("%s_reduce_tree_rate_gem1_Etotalcut_debug.root",the_filename), "RECREATE");
...
//	outTree->Branch("Cer_n",      &Cer_n,   "Cer_n/I"    ); //total hits on the virtual plane of GEM00 
	...
	double px_max=0,py_max=0,pz_max=0,p_max=0,theta_max=0;
	double SC0_p_max=0;
	double SC1_p_max=0;
	double SC2_p_max=0;
	double SC3_p_max=0;
...
//			else if(flux_id->at(j)==30) hit_id=16; //
			...
			//if(flux_tid->at(j)==1 ){
				...
				//if(hit_id==2 && flux_mtid->at(j)==0 && flux_tid->at(j)==1 && hit_pf>190 /*&& abs(flux_avg_z->at(j))<4 && abs(flux_avg_ly->at(j))<10*/ ){
				//if(hit_id==2 && flux_mtid->at(j)==0 && flux_tid->at(j)==1 && hit_pf>190 /*&& abs(flux_avg_z->at(j))<4 && abs(flux_avg_ly->at(j))<10*/ ){
				if(hit_id==4 /*&& flux_mtid->at(j)==1 && flux_tid->at(j)<=3 && abs(flux_avg_z->at(j))<4 && abs(flux_avg_ly->at(j))<10*/ ){
				//if(hit_id==3 /*&& abs(flux_pid->at(j))==11*/ && abs(flux_avg_lx->at(j)/1e1)<2.5 && abs(flux_avg_ly->at(j)/1e1)<5){
                        ...
                        //cout<<"event="<<i<<"index="<<j<<"mtid="<<flux_mtid->at(j)<<"tid="<<flux_tid->at(j)<<"mpid_flux="<<flux_mpid->at(j)<<"flux_pid="<<flux_pid->at(j)<<"vz="<<flux_avg_z->at(j)<<"lz="<<flux_avg_lz->at(j)<<"flux_p="<<hit_pf<<endl;
                                Is_prime=1;
				if (E >= edep_6p1_max){
					edep_6p1_max=E;
					p_max=hit_pf;
