...
int analysis_eAll_gem_prime(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	//sprintf(outputfile_name, "%s_output_gem_gammaE_prime_pL1GeV.root",the_filename);
	sprintf(outputfile_name, "%s_output_ec_electronE_prime_pL100MeV.root",the_filename);
	...
	TH2F *hgen_ThetaP_gem2=new TH2F("gen_ThetaP_gem2","flux events; #theta (deg); P (GeV)",360,0,180,1000,0,1000);  
	TH2F *hgen_Thetavz_gem2=new TH2F("gen_Thetavz_gem2","flux events; #theta (deg); #phi (deg)",360,0,180,2080,-40,1000);     
	TH2F *hgen_Thetavz_gem2_tid1=new TH2F("gen_Thetavz_gem2_tid1","flux events; #theta (deg); #phi (deg)",360,0,180,2080,-40,1000);     
	TH1F *hgen_Theta_gem2=new TH1F("gen_Theta_gem2","flux events; #theta (deg); #phi (deg)",3600,0,180);     
	TH1F *hgen_Theta_gem2_tid1=new TH1F("gen_Theta_gem2_tid1","flux events; #theta (deg); #phi (deg)",3600,0,180);     
	...
	TH2F *hgen_ThetaP_gem1=new TH2F("gen_ThetaP_gem1","flux events; #theta (deg); P (GeV)",360,0,180,1000,0,1000);  
	TH2F *hgen_Thetavz_gem1=new TH2F("gen_Thetavz_gem1","flux events; #theta (deg); #phi (deg)",360,0,180,2080,-40,1000);     
	TH2F *hgen_Thetavz_gem1_tid1=new TH2F("gen_Thetavz_gem1_tid1","flux events; #theta (deg); #phi (deg)",360,0,180,2080,-40,1000);     
	TH1F *hgen_Theta_gem1=new TH1F("gen_Theta_gem1","flux events;vertex #theta (deg); #phi (deg)",3600,0,180);     
	TH1F *hgen_Theta_gem1_tid1=new TH1F("gen_Theta_gem1_tid1","flux events;vertex #theta (deg); #phi (deg)",3600,0,180);     
	...
	TH1F *hhit_Edepgem1_tid1=new TH1F("hit_Edepgem1_tid1","gem1;log10(Edep)(MeV);rate(Hz)",100,-4,3);
	...
	TH1F *hhit_E_gem1_tid1=new TH1F("hit_E_gem1_tid1","gem1;log10(Edep)(MeV);rate(Hz)",140,-4,3);
	TH1F *hhit_E_gem2_tid1=new TH1F("hit_E_gem2_tid1","gem2;log10(Edep)(MeV);rate(Hz)",140,-4,3);
	...
	TH1F *hhit_ECend_E_tid1=new TH1F("hit_ECend_E_tid1","flux;log10(E)(MeV);rate(Hz)",140,-3,4);
	TH1F *hhit_Egem2=new TH1F("hit_Egem2","flux;log10(E)(MeV);rate(Hz)",140,-3,4);
	TH1F *hhit_Egem1_tid1=new TH1F("hit_Egem1_tid1","flux;log10(E)(MeV);rate(Hz)",140,-3,4);
	TH1F *hhit_Egem2_tid1=new TH1F("hit_Egem2_tid1","flux;log10(E)(MeV);rate(Hz)",140,-3,4);
	...
	TH2F *vertexZ_vs_vertexR_tid1_hist=new TH2F("vertexZ_vs_vertexR_tid1_hist","vertex;z (cm);R(cm)",2080,-40,1000,1600,0,800);	
	TH2F *vertexZ_vs_vertexR_gem2_tid1_hist=new TH2F("vertexZ_vs_vertexR_gem2_tid1_hist","vertex;z (cm);R(cm)",2080,-40,1000,1600,0,800);	
	...
	double hit_lx_max=0;
	double hit_ly_max=0;
	double hit_lz_max_tid1=0;
	double hit_lx_max_tid1=0;
	double hit_ly_max_tid1=0;
	double hit_th_max=0;
	double hit_vz_max=0;
	double hit_vr_max=0;
	...
	double trackE_flux_tid1=0;
	double hit_th_max_tid1=0;
	double hit_vz_max_tid1=0;
	double hit_vr_max_tid1=0;
	double hit_phi_max_tid1=0;
	double edep_6p1_max_tid1= 0;
	double trackE_flux_gem2_tid1=0;
	double edep_6p1_max_gem2_tid1= 0;
	double hit_th_max_gem2_tid1=0;
	double hit_phi_max_gem2_tid1=0;
	double hit_vz_max_gem2_tid1=0;
	double hit_vr_max_gem2_tid1=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		hit_ly_max_tid1=0;
		trackE_flux_tid1=0;
		trackE_flux_gem2_tid1=0;
		hit_th_max_tid1=0;
		hit_th_max_gem2_tid1=0;
		hit_vz_max_tid1=0;
		hit_vz_max_gem2_tid1=0;
		hit_vr_max_tid1=0;
		hit_vr_max_gem2_tid1=0;
		hit_phi_max_tid1=0;
		hit_phi_max_gem2_tid1=0;
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			double Edep=E=flux_trackE->at(j);		  
			...
			if (hit_id==3) {//gem1
				...
				N_gem00 ++;
				//if(flux_pid->at(j)==22 && flux_pz->at(j)>0){  
				if(flux_pid->at(j)==11 && flux_pz->at(j)>0){  
					//if (hit_p >= edep_6p1_max){
					if (flux_tid->at(j)==1){
						edep_6p1_max_tid1=hit_p;
						trackE_flux_tid1 = Edep;
                                                hit_th_max_tid1 = hit_th;
                                                hit_phi_max_tid1 = hit_phi;
                                                hit_vz_max_tid1 = hit_vz;
                                                hit_vr_max_tid1 = hit_vr;
                                                hit_lz_max_tid1 = flux_avg_lz->at(j)/1e1;
                                                hit_lx_max_tid1 = flux_avg_lx->at(j)/1e1;
                                                hit_ly_max_tid1 = flux_avg_ly->at(j)/1e1;
					}else{
					 if (hit_p >= edep_6p1_max){
						edep_6p1_max=hit_p;
						trackE_flux = Edep;
					...
					//cout<<"event="<<i<<"index="<<j<<"hit_ID="<<hit_id<<" hit_gem00"<<endl;
					Edepsc1 += Edep;
			...
			if (hit_id==5) {//gem2
				//if(flux_pid->at(j)==22 && flux_pz->at(j)>0){
				if(flux_pid->at(j)==11 && flux_pz->at(j)>0){
					//if (hit_p >= edep_6p1_max_gem2){
					if (flux_tid->at(j)==1){
						edep_6p1_max_gem2_tid1=hit_p;
						trackE_flux_gem2_tid1 = Edep;
                                                hit_th_max_gem2_tid1 = hit_th;
                                                hit_phi_max_gem2_tid1 = hit_phi;
                                                hit_vz_max_gem2_tid1 = hit_vz;
                                                hit_vr_max_gem2_tid1 = hit_vr;
					}else{
					 if (hit_p >= edep_6p1_max_gem2){
		...
		tree_solid_gem->GetEntry(i);
			...
			mpid_gem=0;
			pid_gem=0;
			//process_tree_solid_gem(tree_solid_gem,totEdep_gem1,totEdep_gem2,procID_gem,mpid_gem,pid_gem);
		...
		if(Fillgem){
			hgen_ThetaP_gem2->Fill(hit_th_max_gem2,edep_6p1_max_gem2,rate);
			//hgen_ThetaPhi_gem2->Fill(hit_th_max_gem2,hit_phi_max_gem2,rate);                  		
			hgen_PhiP_gem2->Fill(hit_phi_max_gem2,edep_6p1_max_gem2,rate);                  				
			hgen_ThetaPhiP_gem2->Fill(hit_th_max_gem2,hit_phi_max_gem2,edep_6p1_max_gem2,rate);                  	        if(edep_6p1_max_gem2>1000){		
			...
			hhit_Egem2->Fill(log10(trackE_flux_gem2),rate);
		        vertexZ_vs_vertexR_gem2_hist->Fill(hit_vz_max_gem2,hit_vr_max_gem2,rate);
	                hgen_Thetavz_gem2->Fill(hit_th_max_gem2,hit_vz_max_gem2);
                        ...
                        if(edep_6p1_max_gem2_tid1>1000){ 
                        hgen_Theta_gem2_tid1->Fill(hit_th_max_gem2_tid1,rate); 
			hhit_Egem2_tid1->Fill(log10(trackE_flux_gem2_tid1),rate);
		        vertexZ_vs_vertexR_gem2_tid1_hist->Fill(hit_vz_max_gem2_tid1,hit_vr_max_gem2_tid1,rate);
	                hgen_Thetavz_gem2_tid1->Fill(hit_th_max_gem2_tid1,hit_vz_max_gem2_tid1);
                        }      
		...
		if(Fillecal){
			hgen_ThetaP_gem1->Fill(hit_th_max,edep_6p1_max,rate);
			...
			hgen_ThetaPhiP_gem1->Fill(hit_th_max,hit_phi_max,edep_6p1_max,rate);                  			
                        if(trackE_flux_tid1>1 && abs(hit_lx_max_tid1)<5 && abs(hit_ly_max_tid1)<5){
			hhit_ECend_E_tid1->Fill(log10(trackE_flux_tid1),rate);
                        }
                        if(trackE_flux>1 && abs(hit_lx_max)<5 && abs(hit_ly_max)<5){
			hhit_ECend_E->Fill(log10(trackE_flux),rate);
                        }
                        if(edep_6p1_max>1000){
			...
			hhit_Egem1->Fill(log10(trackE_flux),rate);
		        vertexZ_vs_vertexR_hist->Fill(hit_vz_max,hit_vr_max,rate);
	                hgen_Thetavz_gem1->Fill(hit_th_max,hit_vz_max);
                        ...
                        if(edep_6p1_max_tid1>1000){ 
                        //if(hit_th_max_tid1==0){ 
			//cout<<"event="<<i<<"edep_6p1_max_tid1="<<edep_6p1_max_tid1<<"trackE_flux="<<trackE_flux<<"th_max_tid1="<<hit_th_max_tid1<<"phi="<<hit_phi_max<<endl;
                       // }
                        hgen_Theta_gem1_tid1->Fill(hit_th_max_tid1,rate);  
			hhit_Egem1_tid1->Fill(log10(trackE_flux_tid1),rate);
		        vertexZ_vs_vertexR_tid1_hist->Fill(hit_vz_max_tid1,hit_vr_max_tid1,rate);
	                hgen_Thetavz_gem1_tid1->Fill(hit_th_max_tid1,hit_vz_max_tid1);
                        }     
