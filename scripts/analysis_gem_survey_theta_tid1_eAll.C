...
int analysis_gem_survey_theta_tid1_eAll(string inputfile_name,string runmode="trigger",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_preshower_reconstructtheta_rate_tid1_EL5GeV_finerbin_debug.root",the_filename);
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			if (hit_id==2 && flux_tid->at(j)==1) {//gem1
                        cout<<"diffx="<<diff_vx<<"diff_vy"<<diff_vy<<"diff_vz="<<flux_avg_z->at(j)/1e1<<endl;
			...
			//		cout<<"event="<<i<<"index="<<j<<"trackE_flux="<<trackE_flux<<" hit_gem00"<<endl;
					Edepsc1 += Edep;
					//procID_flux=flux_procID->at(j);
