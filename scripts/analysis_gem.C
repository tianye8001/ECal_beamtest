...
int analysis_gem(string inputfile_name,string runmode="trigger", bool Is_tellorig=false,string filetype="",bool Is_new=true){
	...
	sprintf(outputfile_name, "%s_output_preshower_electronE_pzcut_e.root",the_filename);
	//sprintf(outputfile_name, "%s_output_ec_gammaE_pzcut.root",the_filename);
	...
	TH1F *hgen_Theta_gem2=new TH1F("gen_Theta_gem2","flux events; #theta (deg); rate (Hz)",720,0,180);     
	...
	TH1F *hgen_Theta_gem1=new TH1F("gen_Theta_gem1","flux events;vertex #theta (deg); #rate (Hz)",720,0,180);     
	...
	TH1F *hhit_E[n],*hhit_pf[n],*hhit_E_mip[n],*hhit_E_photonele[n],*hhit_E_ele[n],*hhit_Edep[n];		  	
	for(int i=0;i<n;i++){
		char hstname[100];
		...
		sprintf(hstname,"hit_pf_%i",i);
		hhit_pf[i]=new TH1F(hstname,detname[i].c_str(),1100,0,11);
	...
	//TH1F *hE_gem1=new TH1F("hE_gem1","gem1;track_E (MeV);rate(Hz)",201,-0.5,50.5);
	...
	TH1F *hhit_Edepgem1_MIP=new TH1F("hit_Edepgem1_MIP","gem1;Edep(MeV);rate(Hz)",100,0,0.2);
	TH1F *hhit_Edepgem1_gemgas=new TH1F("hit_Edepgem1_gemgas","gem1;Edep(MeV);rate(Hz)",250,0,1);
	...
	TH1F *hhit_E_gem1_Moller=new TH1F("hit_E_gem1_Moller","gem1;log10(Edep)(MeV);rate(Hz)",140,-3,4);
	TH1F *hhit_E_gem1_Compton=new TH1F("hit_E_gem1_Compton","gem1;log10(Edep)(MeV);rate(Hz)",140,-3,4);
	...
	TH1F *hhit_ECend_p=new TH1F("hit_ECend_p","flux;p (MeV/c);rate(Hz)",3000,0,6000);
	TH1F *hhit_ECend_E=new TH1F("hit_ECend_E","flux;log10(E)(MeV);rate(Hz)",140,-3,4);
	TH2F *vertexZ_vs_vertexR_EC_hist=new TH2F("vertexZ_vs_vertexR_EC_hist","#gamma in front of SC3; vertex Z (cm);vertex R(cm)",1270,-40,2500,750,0,1500);	
	TH2F *vertexZ_vs_E_EC_hist=new TH2F("vertexZ_vs_E_EC_hist","#gamma in front of SC3;vertex Z (cm);E(MeV)",1270,-40,2500,500,0,1000);	
	TH1F *vertexZ_EC_hist=new TH1F("vertexZ_EC_hist","#gamma in front of SC3;vertex Z (cm)",1270,-40,2500);	
	TH1F *vertexR_EC_hist=new TH1F("vertexR_EC_hist","#gamma in front of SC3;vertex R (cm)",750,0,1500);	
	...
	TH1F *vertex_Z_hist=new TH1F("vertex_Z_hist","vertex;z (cm);rate(Hz)",83200,-40,1000);	
	TH2F *vertexZ_vs_vertexR_hist=new TH2F("vertexZ_vs_vertexR_hist","vertex;z (cm);R(cm)",3080,-40,1500,3000,0,1500);	
	//TH2F *vertexZ_vs_vertexR_hist=new TH2F("vertexZ_vs_vertexR_hist","vertex;z (cm);R(cm)",3080,-40,1500,500,0,5000);	
	TH2F *vertexX_vs_vertexY_hist=new TH2F("vertexX_vs_vertexY_hist","vertex;X (cm);Y(cm)",3600,-20,160,3600,-20,160);	
...
//	TH3F *vertexXYZ_gem2_hist=new TH3F("vertexXYZ_gem2_hist","vertex;X (cm);Y(cm);Z (cm)",360,-20,160,360,-20,160,1020,-20,1000);	
	TH2F *vertexZ_vs_vertexR_gem2_hist=new TH2F("vertexZ_vs_vertexR_gem2_hist","vertex;z (cm);R(cm)",3080,-40,1500,3000,0,1500);	
	...
	TH2F *vertexZ_vs_vertexR_cut_hist=new TH2F("vertexZ_vs_vertexR_cut_hist","vertex;z (cm);R(cm)",3080,-40,1500,3000,0,1500);	
	...
	TH1F *vertex_R_hist=new TH1F("vertex_R_hist","vertex;R (cm);rate (Hz)",3000,0,1500);	
	...
	TH2F *hitZ_vs_hitR_hist=new TH2F("hitZ_vs_hitR_hist","gem1;z (cm);R (cm)",120,200,260,80,720,760);	
        TH1F *E_electron_hist =new TH1F("E_electron_hist","gem1; flux_E (MeV);rate (Hz)",600,0,300);
        TH1F *E_positron_hist =new TH1F("E_positron_hist","gem1; flux_E (MeV);rate (Hz)",600,0,300);
        TH2F *E_positron_vs_electron_hist =new TH2F("E_positron_vs_positron_hist","gem1; electron_E (MeV);positron_E (MeV)",600,0,300,600,0,300);
	...
	Double_t totEdep_gem1_gas[6]={0};
	Double_t totEdep_gem2_gas[6]={0};
	Double_t totEdep_gem1tot=0;
	Double_t totEdep_gem2tot=0;
	int procID_gem=0;
	int mpid_gem=0;
	int pid_gem=0;
	int procID_flux=0;
	...
	double hit_vr_max_gem2=0;
        double E_electron=0;
        double E_positron=0;
	int mpid_max=0,mpid_max_gem2=0,pid_max=0;
	...
	for(long int i=0;i<N_events;i++){	  		
		...
		//	rate=80e-6/1.6e-19/event_actual/loop_time*add_norm;  //1ua		  
			rate=5e-6/1.6e-19/event_actual/loop_time*add_norm;  //1ua		  
		...
		for (Int_t j=0;j<flux_hitn->size();j++) {
			...
			double hit_p=sqrt(flux_px->at(j)*flux_px->at(j)+flux_py->at(j)*flux_py->at(j)+flux_pz->at(j)*flux_pz->at(j));  //MeV to GeV
	                ...
	                //double hit_th=atan2(hit_r,hit_z)*DEG;
			//double trackE_flux=flux_trackE->at(j);
			...
			int hit_id=-1;
			if(flux_id->at(j)==1) hit_id=0; // SC front,SC0
			else if(flux_id->at(j)==2) hit_id=1; // SC back, SC1
			...
			else if(flux_id->at(j)==11) hit_id=16; // SC2 front
			else if(flux_id->at(j)==7) hit_id=7; // spd 1
			else if(flux_id->at(j)==8) hit_id=8; // spd 2	  
			else if(flux_id->at(j)==9) hit_id=9; // shower	  
			else if(flux_id->at(j)==13) hit_id=10; // shower	  
			else if(flux_id->at(j)==20) hit_id=11; // shower	  
			else if(flux_id->at(j)==21) hit_id=12; // shower	  
			else if(flux_id->at(j)==22) hit_id=13; // shower	  
			else if(flux_id->at(j)==23) hit_id=14; // shower	  
			else if(flux_id->at(j)==30) hit_id=15; // shower	  
			...
			if (abs(flux_pid->at(j)) == 11) hhit_E_ele[hit_id]->Fill(E,rate);  
                       // if(flux_mtid->at(j)==1){
                        //cout<<"event="<<i<<"index="<<j<<"mtid="<<flux_mtid->at(j)<<"tid="<<flux_tid->at(j)<<"mpid_flux="<<flux_mpid->at(j)<<"hit_id="<<hit_id<<"pid_flux="<<flux_pid->at(j)<<"E="<<Edep<<"p_flux="<<hit_p<<endl;
                        // }
			if (hit_id==2) {//gem1
	         	//if (hit_id==3) {//gem1
                        //cout<<"event="<<i<<"index="<<j<<"mtid="<<flux_mtid->at(j)<<"tid="<<flux_tid->at(j)<<"mpid_flux="<<flux_mpid->at(j)<<"hit_id="<<hit_id<<"pid_flux="<<flux_pid->at(j)<<"E="<<Edep<<"p_flux="<<hit_p<<endl;
	         	 if(flux_mtid->at(j)==1 && flux_mpid->at(j)==22 && flux_tid->at(j)==2 && flux_pid->at(j)==11){
	                        E_electron_hist->Fill(Edep,rate);
                                E_electron = Edep;	         
                         }        
	         	 if(flux_mtid->at(j)==1 && flux_mpid->at(j)==22 && flux_tid->at(j)==3 && flux_pid->at(j)==-11){
	                        E_positron_hist->Fill(Edep,rate);	         
                                E_positron = Edep;	         
                         }        
				...
				N_gem00 ++;
                                //cout<<"event="<<i<<"index="<<j<<"mtid="<<flux_mtid->at(j)<<"tid="<<flux_tid->at(j)<<"mpid_flux="<<flux_mpid->at(j)<<"pid_flux="<<flux_pid->at(j)<<"E="<<Edep<<"p_flux="<<hit_p<<endl;
				//if(flux_pid->at(j)==11 && flux_pz->at(j)>0 /*&& abs(hit_vz)<5*/){  
				if(abs(flux_pid->at(j))==11 &&  flux_pz->at(j)>0 /*&& abs(hit_vz)<5*/){ 
					...
					if (hit_p >= edep_6p1_max){
						...
						trackE_flux = Edep;
                                                ...
                                                hit_vx_max = hit_vx;
                                                hit_vy_max = hit_vy;
                                                hit_lz_max = flux_avg_lz->at(j)/1e1;
                                                hit_lx_max = flux_avg_lx->at(j)/1e1;
                                                hit_ly_max = flux_avg_ly->at(j)/1e1;
					...
					if(/*flux_procID->at(j)==50*/hit_p>40 && flux_pz->at(j)>0 ){
				...
				//	if(procID_flux<10){
				//		hhit_Egem1_procID[procID_flux]->Fill(log10(hit_p),rate);
				//	}else{
				//		hhit_Egem1_procID_other->Fill(log10(hit_p),rate);
				//	}
					Fillecal=kTRUE;
				} else{// end pid=22
					Fillecal=kFALSE;
				...
				if(flux_pid->at(j)==11 && flux_pz->at(j)>0 /*&& abs(hit_vz)<5*/){
					if (hit_p >= edep_6p1_max){
						...
						trackE_flux_gem2 = Edep;
                                                ...
                                                mpid_max_gem2=flux_mpid->at(j);
					}
					//cout<<"event="<<i<<"index="<<j<<"hit_ID="<<hit_id<<" hit_gem10"<<endl;
			...
			//cout<<"event="<<i<<"index="<<j<<"hit_ID="<<hit_id<<" hitx_gem00="<<hitx_gem00<<"hity_gem00="<<hity_gem00<<" hitx_gem10="<<hitx_gem10<<"hity_gem10="<<hity_gem10<<endl;
			//if(N_gem00>0 && N_gem10>0){
			//}
		}	// end of flux	
		//if(hitx_gem00 !=0 || hitx_gem10 != 0){	
		//cout<<"event="<<i<<" hitx_gem00="<<hitx_gem00<<"hity_gem00="<<hity_gem00<<" hitx_gem10="<<hitx_gem10<<"hity_gem10="<<hity_gem10<<endl;
		if(N_gem00>0 || N_gem10>0){
		...
		if(N_gem00>0 && totEdep_gem1tot>0){
				hhit_Edepgem1->Fill(log10(totEdep_gem1tot),rate);
				hhit_Edepgem1_MIP->Fill(totEdep_gem1tot,rate);
				hhit_Edepgem1_gemgas->Fill(totEdep_gem1_gas[1],rate);
               }
		if(N_gem10>0 && totEdep_gem2tot>0){
                ...
                if(Fillgem && Fillecal){
                 ...
                 // cout<<"event="<<i<<"hit on GEM1 and GEM2"<<endl;
                }
		if(Fillgem){
			...
			hhit_Egem2->Fill(log10(trackE_flux_gem2),rate);
		        ...
		        //vertexXYZ_gem2_hist->Fill(hit_vx_max_gem2,hit_vy_max_gem2,hit_vz_max_gem2,rate);
	                hgen_Thetavz_gem2->Fill(hit_th_max_gem2,hit_vz_max_gem2);
			if(procID_flux<10){
			 hhit_Egem2_procID[procID_flux]->Fill(log10(trackE_flux_gem2),rate);
			}else{
			 hhit_Egem2_procID_other->Fill(log10(trackE_flux_gem2),rate);
			}
			if(totEdep_gem2tot>0 /*&& pid_gem==22*/){
				hhit_E_gem2->Fill(log10(totEdep_gem2tot),rate);
                               for(int t=0;t<6;t++){
		...
		if(Fillecal){
			...
			hgen_PhiP_gem1->Fill(hit_phi_max,edep_6p1_max,rate);        
                        E_positron_vs_electron_hist->Fill(E_electron, E_positron,rate);          				
			...
			hE_gem1->Fill(trackE_flux,rate);
                        if(trackE_flux>0 && pid_max==11 && abs(hit_lx_max)<5 && abs(hit_ly_max)<5){
			hhit_Egem1->Fill(log10(trackE_flux),rate);
			hhit_ECend_p->Fill(edep_6p1_max,rate);
			hhit_ECend_E->Fill(log10(trackE_flux),rate);
		        vertexZ_vs_vertexR_EC_hist->Fill(hit_vz_max,hit_vr_max,rate);
		        vertexZ_vs_E_EC_hist->Fill(hit_vz_max,trackE_flux,rate);
		        vertexZ_EC_hist->Fill(hit_vz_max,rate);
		        vertexR_EC_hist->Fill(hit_vr_max,rate);
                        }
                        if(mpid_max==11){
			hhit_E_gem1_Moller->Fill(log10(trackE_flux),rate);
                        }
                        if(mpid_max==22){
			hhit_E_gem1_Compton->Fill(log10(trackE_flux),rate);
                        }
		        vertexZ_vs_vertexR_hist->Fill(hit_vz_max,hit_vr_max,rate);
			vertexZ_vs_E_hist->Fill(hit_vz_max,trackE_flux,rate);
			...
			if(totEdep_gem1tot>0 /*&& pid_gem==22*/){
			...
			if(true){
				...
				/*	TCanvas *c_cc = new TCanvas("cc", "cc",1600,900);
					...
					c_EC->cd(2);
					hec_Eend_shower->Draw();
					c_EC->cd(3);
					hec_Eend_preshower->Draw();
					...
					c_gem[1]->cd(2);
					// gPad->SetLogy();
					//mvertexZ_vs_E_hist->SetTitle("e^{+} Annihilation");
					mvertexZ_vs_E_cut_hist->SetMarkerColor(2);
					// mvertexZ_vs_E_hist->GetYaxis()->SetRangeUser(0,2);
					// mvertexZ_vs_E_hist->SetTitle("gamma conversion");
					...
					// c_gem[1]->cd(3);
					// mvertexZ_vs_mvertexR_hist->Draw("COLZ");
					// c_gem[1]->cd(4);
					// mvertexZ_vs_mvertexR_cut_hist->Draw("COLZ");

					c_gem[4] = new TCanvas("c_gem[4]", "gem1",1800,1000);
					gPad->SetLogy();
					//c_gem[2]->Divide(1,2);
					// c_gem[2]->cd(1);
					hprocid_gem1->GetYaxis()->SetRangeUser(1e0,5e6);
					...
					//c_gem[2]->cd(2);
					//TH1F *hprocid_gem1_clone = hprocid_gem1->Clone();
					//hprocid_gem1_clone->GetXaxis()->SetRangeUser(0,10);
					//hprocid_gem1_clone->Draw("HIST");
					...
					hhit_Egem1_procID[1]->GetYaxis()->SetRangeUser(1e-1,1e6);
					...
					TLegend *leg12 = new TLegend(0.6,0.4,0.85,0.8);
					...
					leg1->SetFillColor(0);
					leg1->Draw("text same");
					c_gem[6] = new TCanvas("c_gem[6]", "gem1_depositE",1800,1000);
					gPad->SetLogy();
					hhit_Edepgem1_procID[1]->SetLineColor(1);
					hhit_Edepgem1_procID[1]->GetYaxis()->SetRangeUser(1e-1,1e6);
					...
					c_gem[7] = new TCanvas("c_gem[7]", "gem1_depositE",1800,1000);
					gPad->SetLogy();
					...
					hhit_Edepgem1_electron->SetLineColor(4);
					...
					leg11->AddEntry(hhit_Edepgem1_electron,"abs(mpid)=11","l");
					...
					gSystem->Exec("pdfunite ./c*.pdf ./HallC_BeamOnTarget_Hallair_gem1_electron.pdf");
